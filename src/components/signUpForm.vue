...
export default {    
    ...
    data:function(){
        return {
            ...
            countryRules:[
                v => !!v || "Please select a country"
            ],
            dateRules: [
                v => !!v || "Please Select your birth date"
            ],
            passwordRules: [
                v => /(?=.*[a-z])/.test(v) || "Password must have at least one lowercase character",
                v => /(?=.*[0-9])/.test(v) || "Password must contain at least one number",
                v => /(?=.{8,})/.test(v) || "Password must be eight characters or longer",
            ]
        }
    },
    watch: {
